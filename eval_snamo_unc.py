...
eval_snamo_unc.py
------------------
Runs the S-NAMO simulator WITH the uncertainty model enabled (pure_snamo=False)
across all 9 benchmark configs, for a given number of trials (default: 1).
Supports PyBullet GUI visualization.
Saves results to results/evaluation_tables/snamo_unc_eval_results.json.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate S-NAMO (Uncertainty-Aware) in PyBullet")
    ...
    parser.add_argument("--push-fail-rate", type=float, default=0.0, help="Probability that a push attempt fails")
    ...
    print("\n" + "=" * 70)
    print(f"  S-NAMO (Uncertainty-Aware) Batch Evaluation  |  trials={args.trials}  fail_rate={args.push_fail_rate}  gui={args.gui}")
    print("=" * 70)
    ...
    for name, cfg_rel in BENCHMARK_CONFIGS:
        ...
        for t in range(args.trials):
            ...
            try:
                # Run with pure_snamo=False (enable uncertainty model) and pass fail rate
                res = run_simulation(cfg_path, gui=args.gui, pure_snamo=False, push_fail_rate=args.push_fail_rate)
    ...
    out_path = out_dir / "snamo_unc_eval_results.json"
    with open(out_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"\n[S-NAMO Uncertainty Eval] Results saved to {out_path}")
